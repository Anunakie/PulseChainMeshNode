// PulseMesh Browser - Token Payment Manager
...
// Default placeholder token address (update when real PulseMesh token is deployed)
...
function getTokenContract() {
    ...
    if (addr === DEFAULT_TOKEN_ADDRESS) {
        throw new Error('Token address not configured. Set a valid PulseMesh token address.');
    }
    const p = getProvider();
...
async function getTokenBalance() {
    try {
        ...
        const addr = getTokenAddress();
        if (addr === DEFAULT_TOKEN_ADDRESS) {
            return { balance: '0', formatted: '0.0', symbol: 'PMESH', configured: false };
        }
        const contract = getTokenContract();
        const [balance, symbol, decimals] = await Promise.all([
            contract.balanceOf(wallet.address),
            contract.symbol().catch(() => 'PMESH'),
            contract.decimals().catch(() => 18),
        ]);
        ...
        return { balance: '0', formatted: '0.0', symbol: 'PMESH', error: err.message };
    }
}
...
async function sendTokens(to, amount) {
    ...
    const symbol = await contract.symbol().catch(() => 'PMESH');
